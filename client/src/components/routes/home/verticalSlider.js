...
const VerticalSlider = () => {
  ...
  const renderImageList = (imageList) => {
    ...
    imageList = imageList.filter(
      (image) => image.imageLocalPath.search('icon') === -1,
    );

    return imageList.map(({ id, imageLocalPath, imageURL }) => {
      return <img key={id} src={imageURL} alt={imageLocalPath} />;
    });
  };

  return (
    <Swiper {...params}>{renderImageList(homeAPIData.data.carousels)}</Swiper>
  );
};
