...
const TopCategoriesAndBrands = () => {
  ...
  const renderImageList = (imageList, filterQueryType) => {
    return imageList.map((info) => {
      ...
      return (
        ...
        >
          <Link
            to={`/products?q=${filterQuery}::page=0,${MAX_PRODUCTS_PER_PAGE}`}
          >
            <img
              src={info.imageURL}
              alt={info.imageLocalPath}
  ...
  return (
    <>
      {renderCategoryAndBrandsList(
        '#Shop Top Brands',
        homeAPIData.data.brands,
        queryType.brand,
      )}
      {renderCategoryAndBrandsList(
        '#Shop Top Categories',
        homeAPIData.data.apparels,
        queryType.apparel,
      )}
    </>
  );
