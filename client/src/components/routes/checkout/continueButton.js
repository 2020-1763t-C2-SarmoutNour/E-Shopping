...
function ContinueButton(props) {
  const renderContinueDesktopBtn = () => {
    return (
      <Grid
        ...
        container
        justify="flex-end"
        style={{ padding: '30px 40px 30px 0' }}
      >
        <Button
          variant="contained"
          size="large"
  ...
  return (
    <>
      <Hidden xsDown>{renderContinueDesktopBtn()}</Hidden>

      <Hidden smUp>{renderContinueMobileBtn()}</Hidden>
    </>
  );
}
...
const continueButtonMemoWrapper = React.memo(
  ContinueButton,
  continueButtonPropsAreEqual,
);
