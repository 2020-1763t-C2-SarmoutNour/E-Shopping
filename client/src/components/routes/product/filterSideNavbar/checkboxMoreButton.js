...
export default function CheckboxMoreButton(props) {
  const [moreButtonState, setMoreButtonState] = useState({
    active: false,
    topPosition: 0,
  });
  ...
  const renderMoreButtonList = () => {
    return (
      <Paper elevation={3} variant="outlined" square style={paperStyles}>
        <Grid item container xs={props.size} direction="row" style={gridStyles}>
          <Grid item sm={11}>
            <SortedCheckboxList
              attrList={props.checkboxList}
              title={props.title}
          ...
          <Grid
            ...
            container
            justify="flex-end"
            style={{ height: '5%', paddingRight: '0.5rem' }}
          >
            <IconButton
              size="medium"
              color="primary"
