...
function TabPanel(props) {
  ...
  const renderDataList = (brandList, queryParam) => {
    return brandList.map(({ id, value }) => {
      return (
        <Link
          key={id}
          to={`/products?q=${queryParam}=${id}::page=0,${MAX_PRODUCTS_PER_PAGE}`}
  ...
  return (
    ...
    >
      {value === index &&
        renderTabPanel(
          tabsData[TAB_CONFIG[index].mapKey].brands,
...
export const TabPanelList = () => {
  ...
  return <div className={classes.root}>{renderTabPanels()}</div>;
};
