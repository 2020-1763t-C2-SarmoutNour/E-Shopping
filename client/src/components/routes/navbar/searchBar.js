  ...
  const searchKeyword = (value) => {
    if (value && !value.isEmpty) {
      ...
      for (let index = 0; index < searchSuggestions.data.length; ++index) {
        if (
          searchSuggestions.data[index].keyword.length === value.length &&
    ...
    return (
      <TextField
        ...
        InputProps={{
          ...params.InputProps,
          startAdornment: (
            <ArrowBackIcon onClick={props.handleClose} fontSize="large" />
          ),
        }}
      />
    );
  ...
  return (
    <Grid container alignItems="center">
      <Autocomplete
        ...
        renderInput={(params) =>
          props.device
            ? renderMobileTextField(params)
            : renderDesktopTextField(params)
        }
      />
      {isLoading ? (
        <StyledSearchBarDimmer active inverted>
          <Loader inverted>Loading</Loader>
        </StyledSearchBarDimmer>
      ) : null}
    </Grid>
  );
}

export default connect(null, { getSearchSuggestions, getDataViaAPI })(
  SearchBar,
);
